...
const EditRestaurantNotification = () => {
  const navigate = useNavigate();
  const { id } = useParams(); // Get the notification ID from the URL
  const [formData, setFormData] = useState({
    title: "Sample Restaurant Notification", // Pre-filled data for demo
    message: "Our new restaurant offers amazing deals.",
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(`Restaurant Notification ${id} Updated:`, formData);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-md shadow-md">
        <h2 className="text-2xl font-semibold mb-4">Edit Restaurant Notification</h2>
        <div className="mb-4">
...
export default EditRestaurantNotification;
